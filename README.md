## Hi there 👋, I'm RANGANATHA R ⚡
> **Data Analyst | Turning Data into Actionable Insights | Problem Solver**
> 
> **"Transforming complex datasets into clear, actionable stories."**

---

### 🛠️ Tech Stack & Toolbox


| Tool | technical Skills |
| :--- | :--- |
| <img src="https://githubusercontent.com" alt="sql" width="30" height="30"/> **SQL** | Deep Querying, Joins, CTEs, Data Cleaning |
| <img src="https://githubusercontent.com" alt="python" width="30" height="30"/> **Python** | Pandas, Numpy, Deep Understanding of Logic |
| <img src="https://githubusercontent.com" alt="powerbi" width="30" height="30"/> **Power BI** | Interactive Dashboards, Data Modeling |
| <img src="https://icons8.com" width="30" height="30"/> **Excel** | Regression Analysis, Pivot Tables, Formulas |
| <img src="https://icons8.com" width="30" height="30"/> **Documentation** | MS Word, PowerPoint, Detailed Reporting |
| <img src="https://icons8.com" width="30" height="30"/> **Statistics** | Regression Analysis, Correlation, Hypothesis Testing |

---

### 🚀 Professional Focus
* 🔍 **Current Focus:** Deepening my expertise in **Python automation** and **Advanced SQL** optimization.
* 🤖 **Machine Learning:** Beginning to understand the fundamentals of ML
* 💼 **Open to Opportunities:** Actively looking for **Data Analyst** or **Business Analyst** roles.

---

### 📁 Featured Projects
* 🛒 **[Customer-Segmentation](https://github.com)** - RFM analysis for marketing strategy.
* 📈 **[Profit_Analysis](https://github.com)** - Multiple Linear Regression in Excel/Power BI.
* 🎬 **[SQL_CaseStudy_on_IMDB_Dataset](https://github.com)** - Solving business puzzles with complex queries.

---
📫 **Let's Connect:**(https://www.linkedin.com/in/ranganatha-r-0a0812280/)

